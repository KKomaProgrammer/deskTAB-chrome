...
ROOT_FIX="$TMP_BASE/desktab-root-fix-v15.sh"
...
# Some earlier installs contain the real Chrome binary but lost the launcher symlink.
...
# Prefer normal proot-distro, but keep a raw PRoot fallback for already-valid rootfs.
PD_OK=0
...
progress 93 12 "앱 반복 실행 및 D-Bus 세션 수리"
...
# Required invariants. Do not report repair success unless these exist.
...
run_guest_now /bin/bash /tmp/desktab-root-fix-v15.sh || {
  rm -f "$ROOT_FIX"
  fail "XFCE/Chrome 반복 실행 수리 실패"
}
rm -f "$ROOT_FIX"

progress 96 7 "지속 세션 및 성능 실행기 구성"
...
exec "$STATE_DIR/guest-exec.sh" /bin/bash -lc '
  ...
  export XDG_RUNTIME_DIR=/tmp/runtime-root
  export PULSE_SERVER=127.0.0.1
  if [ -r /tmp/desktab-session.env ]; then . /tmp/desktab-session.env; fi
...
sleep 0.35
if command -v termux-x11-preference >/dev/null 2>&1 && command -v timeout >/dev/null 2>&1; then
  timeout 3 termux-x11-preference displayResolutionMode=scaled displayScale=75 fullscreen=true >/dev/null 2>&1 || true
fi
...
# One probe only. A healthy XFCE session is never killed just to open Chrome.
if "$GUEST" /bin/bash -lc 'pgrep -x xfce4-session >/dev/null 2>&1'; then
  exec "$OPEN_CHROME"
fi

rm -f "$TMP_BASE/desktab-session.env"
if ! pgrep -f '[t]ermux-x11 :1' >/dev/null 2>&1; then
  termux-x11 :1 -xstartup "$XSTARTUP" >"$STATE_DIR/x11.log" 2>&1 &
else
  "$XSTARTUP" >"$STATE_DIR/session.log" 2>&1 &
fi

for _ in $(seq 1 50); do
  [ -s "$TMP_BASE/desktab-session.env" ] && break
  sleep 0.2
done
exec "$OPEN_CHROME"
EOF
...
# Host-side launch scripts must be syntactically valid before marking ready.
bash -n "$GUEST_EXEC"
bash -n "$XSTARTUP"
bash -n "$OPEN_CHROME"
...
printf '%s\n' '5' > "$STATE_DIR/desktop-repair-version"
