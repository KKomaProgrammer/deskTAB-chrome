...
# Publish a heartbeat before the first PRoot command. This prevents the Android
# startup watchdog from confusing a slow PRoot startup with a failed RUN_COMMAND.
...
printf '%s\n' '3' > "$STATE_DIR/desktop-repair-version"
printf '%s\n' '11' > "$STATE_DIR/engine-version"
touch "$STATE_DIR/ready"

progress 100 0 "데스크톱 반복 실행/heartbeat 수리 완료"
